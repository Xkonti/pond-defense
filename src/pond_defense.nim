...
game:
  ...
  gameLoop:
    ...
    update:
      ...
      if not isFrogDead and not showMenu:
        # Licking
        if isKeyPressed(Z):
          isTongueOpen = true
    ...
    draw:
      ...
      if showMenu:
        ...
        # Draw UI
        drawTextWithShadow("Points: " & $points, 10, 10, 40, 3, White, Black)
        drawMeter("Moisture", Rectangle(x: 10, y: 60, width: 250, height: 20), frogMoisture, 1.0, Blue)
        drawMeter("Energy", Rectangle(x: 10, y: 140, width: 250, height: 20), frogSatiety, 1.0, Yellow)
